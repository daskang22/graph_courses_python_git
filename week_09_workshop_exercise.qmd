...
total_hiv_raw = pd.read_csv("data/hiv_incidence.csv")
...
hiv_data_combined = pd.merge(new_hiv_long,total_hiv_long,how='outer', on=['country', 'country_code', 'year'])
...
filtered_population = population[population['indicator'] == 'SP.URB.TOTL']

population_long = filtered_population.melt(
    id_vars=['country', 'indicator'],
    var_name='year',
    value_name='population'
).sort_values(by=['country', 'year'])

population_long = population_long.rename(columns={'country': 'country_code'})  # Rename the column
population_long = population_long.drop(columns=['indicator'])  # Drop the indicator column
...
selected_countries = ['United States', 'Zimbabwe', 'South Africa']
...
graph_1 = px.line(
    ...
    labels={'year': 'Year', 'Incidence Rate': 'Incidence Rate'},
)
...
graph_2 = px.line(
    ...
    labels={'year': 'Year', 'Prevalence Rate': 'Prevalence Rate'},
)
